...
const App: React.FC = () => {
  ...
  const exportReport = () => {
    ...
    const report = `
NEUROREDDIT VALIDATION REPORT
${data.metadata.analysis_id}
Chapter ${data.metadata.chapter_number}
═══════════════════════════════════════════════════════════════

METHODOLOGY
─────────────────────────────────────────────────────────────
- Communities Analyzed: ${data.metadata.subreddits.join(', ')}
- Time Period: ${data.metadata.time_filter}
- Analysis Date: ${data.metadata.analyzed_at}
- Unique Authors: ${data.metadata.unique_authors || 'N/A'}

SUMMARY STATISTICS
─────────────────────────────────────────────────────────────
- Total Mentions: ${totalPosts.toLocaleString()}
- Total Comments: ${totalComments.toLocaleString()}
- Themes Identified: ${data.themes.length}
- Avg Engagement: ${Math.round(totalComments / totalPosts)} comments/post
...
`;
    ...
    a.href = url;
    a.download = `NeuroReddit_Chapter${data.metadata.chapter_number}_${data.metadata.analysis_id.replace(/\s+/g, '_')}.txt`;
